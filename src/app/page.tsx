  ...
  return (
    <div className="font-mono">
      ...
      <section className="mb-12">
        <div className="text-[#804400] mb-2">$ cat /etc/motd</div>
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-[#ffb000] mb-4">
          notreally.work
        </h1>
        <p className="text-[#804400] max-w-2xl leading-relaxed">
      ...
      {/* Projects Directory */}
      <section className="mb-12">
        <div className="flex items-baseline justify-between mb-4 text-[#804400]">
        ...
        <div className="text-[#804400] text-sm mb-2">total {projects.length * 4}</div>
        ...
        <div className="space-y-0">
          {projects.length > 0 ? (
            projects.map((project) => (
              <div 
                key={project.slug} 
                className="grid grid-cols-1 md:grid-cols-[12ch_1fr_12ch_80px] gap-2 md:gap-4 border-b border-[#804400]/30 py-2 hover:bg-[#ffb000]/10 items-baseline text-sm"
              >
                <span className="text-[#804400] hidden md:block">drwxr-xr-x</span>
                <Link 
            ...
            <div className="py-4 text-[#804400]">$ echo "No projects found"</div>
          )}
        </div>
      </section>
      ...
      <section className="mb-12">
        ...
        <div className="text-[#804400] text-sm mb-2">total {logs.length * 4}</div>
        ...
        <div className="space-y-0">
          {logs.length > 0 ? (
            logs.map((log) => (
              <div 
                key={log.slug} 
                className="grid grid-cols-1 md:grid-cols-[12ch_1fr] gap-2 md:gap-4 border-b border-[#804400]/30 py-2 hover:bg-[#ffb000]/10 items-baseline text-sm"
              >
                ...
                >
                  <span className="text-[#804400] md:hidden">-rw-r--r-- </span>
                  {log.date} {log.slug}.md
                </Link>
              </div>
            ))
          ) : (
            <div className="py-4 text-[#804400]">$ echo "No logs found"</div>
          )}
        </div>
      </section>
